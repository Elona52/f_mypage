...
class ReservationIntegrationTest {
	...
	void testGetMyReservations_WithJwtToken() throws Exception {
		...
		mockMvc.perform(get("/api/reservation/my")
				...
				.andExpect(jsonPath("$[0].statusCode").value("RESERVED"))
				.andExpect(jsonPath("$[0].payment.amount").value(50000))
	...
	@Test
	@DisplayName("예약 생성 통합 테스트 (JWT 토큰 포함)")
	void testCreateReservation_WithJwtToken() throws Exception {
		// given - 새로운 스케줄 생성
		Schedule newSchedule = Schedule.builder()
				.exerciseName("헬스 트레이닝")
				.exerciseDate(LocalDateTime.now().plusDays(5))
				.exerciseLocation("홍대점")
				.trainerName("박트레이너")
				.capacity(20)
				.currentCount(0)
				.build();
		...
		String requestBody = objectMapper.writeValueAsString(
				java.util.Map.of(
						...
						"reservedDate", LocalDateTime.now().plusDays(5).toString()
				)
		);
		...
		mockMvc.perform(post("/api/reservation")
				...
				.andExpect(jsonPath("$.exerciseName").value("헬스 트레이닝"))
				.andExpect(jsonPath("$.statusCode").value("RESERVED"))
				.andExpect(jsonPath("$.userId").value("testuser"));
	}
	
	@Test
	@DisplayName("JWT 토큰 없이 예약 생성 시 401 에러")
	void testCreateReservation_WithoutToken() throws Exception {
		// given
		String requestBody = objectMapper.writeValueAsString(
				java.util.Map.of(
						"scheduleId", 1L,
	...
	void testCancelReservation_WithJwtToken() throws Exception {
		...
		// when & then
		mockMvc.perform(post("/api/reservation/" + reservationId + "/cancel")
				.header("Authorization", getAuthorizationHeader()))
				.andDo(result -> {
					// 디버깅: 실제 응답 상태와 내용 출력
